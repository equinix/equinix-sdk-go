/*
Equinix Fabric API v4

Contact: api-support@equinix.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fabricv4

import (
	"encoding/json"
	"fmt"
)

// PortSettingsPackageType Type of Port Package
type PortSettingsPackageType string

// List of PortSettings_packageType
const (
	PORTSETTINGSPACKAGETYPE_STANDARD       PortSettingsPackageType = "STANDARD"
	PORTSETTINGSPACKAGETYPE_UNTAGGED       PortSettingsPackageType = "UNTAGGED"
	PORTSETTINGSPACKAGETYPE_UNLIMITED      PortSettingsPackageType = "UNLIMITED"
	PORTSETTINGSPACKAGETYPE_UNLIMITED_PLUS PortSettingsPackageType = "UNLIMITED PLUS"
)

// All allowed values of PortSettingsPackageType enum
var AllowedPortSettingsPackageTypeEnumValues = []PortSettingsPackageType{
	"STANDARD",
	"UNTAGGED",
	"UNLIMITED",
	"UNLIMITED PLUS",
}

func (v *PortSettingsPackageType) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := PortSettingsPackageType(value)
	for _, existing := range AllowedPortSettingsPackageTypeEnumValues {
		if existing == enumTypeValue {
			*v = enumTypeValue
			return nil
		}
	}

	return fmt.Errorf("%+v is not a valid PortSettingsPackageType", value)
}

// NewPortSettingsPackageTypeFromValue returns a pointer to a valid PortSettingsPackageType
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewPortSettingsPackageTypeFromValue(v string) (*PortSettingsPackageType, error) {
	ev := PortSettingsPackageType(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for PortSettingsPackageType: valid values are %v", v, AllowedPortSettingsPackageTypeEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v PortSettingsPackageType) IsValid() bool {
	for _, existing := range AllowedPortSettingsPackageTypeEnumValues {
		if existing == v {
			return true
		}
	}
	return false
}

// Ptr returns reference to PortSettings_packageType value
func (v PortSettingsPackageType) Ptr() *PortSettingsPackageType {
	return &v
}

type NullablePortSettingsPackageType struct {
	value *PortSettingsPackageType
	isSet bool
}

func (v NullablePortSettingsPackageType) Get() *PortSettingsPackageType {
	return v.value
}

func (v *NullablePortSettingsPackageType) Set(val *PortSettingsPackageType) {
	v.value = val
	v.isSet = true
}

func (v NullablePortSettingsPackageType) IsSet() bool {
	return v.isSet
}

func (v *NullablePortSettingsPackageType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePortSettingsPackageType(val *PortSettingsPackageType) *NullablePortSettingsPackageType {
	return &NullablePortSettingsPackageType{value: val, isSet: true}
}

func (v NullablePortSettingsPackageType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePortSettingsPackageType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
