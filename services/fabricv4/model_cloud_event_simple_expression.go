...
type CloudEventSimpleExpression struct {
	...
	// Possible operators to use on filters:  * `=` - equal  * `!=` - not equal  * `>` - greater than  * `>=` - greater than or equal to  * `<` - less than  * `<=` - less than or equal to  * `BETWEEN` - between  * `IN` - in  * `LIKE` - like  * `ILIKE` - like case-insensitive  * `IS NULL` - is null  * `IS NOT NULL` - is not null
